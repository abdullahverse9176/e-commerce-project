...
export const EditCategoryPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (id) {
      // NOTE: User can connect backend API here:
      // e.g. await axios.patch(`/api/categories/update-category/${id}`, { name: formData.name, slug: formData.slug })
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-indigo-400 font-semibold">Edit: {name || 'Category'}</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        {/* Left 2 Cols: Form */}
        <section aria-label="Edit Category Section" className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl space-y-6">
          <div className="flex items-center gap-3 pb-4 border-b border-slate-800">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">Edit Category</h2>
              <p className="text-xs text-slate-400 mt-0.5">
                Update category details, URL routing slug, and storefront banner
              </p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <label className="flex-1 w-full flex items-center justify-center gap-2.5 border-2 border-dashed border-slate-800 hover:border-indigo-500/80 rounded-2xl p-4 bg-slate-950/60 cursor-pointer transition-all group">
                  ...
                  <span className="text-xs text-slate-300 group-hover:text-indigo-300">
                    {imageFile ? imageFile.name : 'Choose a category image file to replace'}
                  </span>
                  <input
                    type="file"
                    accept="image/*"
            ...
            <div className="pt-6 border-t border-slate-800 flex items-center justify-end gap-3">
              ...
              >
                ...
                <span>{isSubmitting ? 'Updating Category...' : 'Save Changes'}</span>
              </button>
            </div>
          </form>
        </section>
        ...
        <aside aria-label="Category Preview and API Note" className="space-y-6">
          ...
          <div className="bg-slate-900/60 border border-indigo-500/20 rounded-3xl p-6 shadow-xl space-y-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-indigo-400 flex items-center gap-2">
              ...
              <span>Update API Hook</span>
            </h3>
            <p className="text-xs text-slate-300 leading-relaxed">
              When ready, hook your backend update endpoint:
            </p>
            <div className="bg-slate-950 rounded-xl p-3 font-mono text-[11px] text-indigo-300 border border-slate-800">
              <code>PATCH /api/categories/update-category/{id}</code>
            </div>
            <p className="text-[11px] text-slate-400 flex items-center gap-1.5">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
              <span>Passes updated name & slug</span>
            </p>
          </div>
        </aside>
      </div>
